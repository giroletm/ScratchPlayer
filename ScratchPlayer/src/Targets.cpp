...
Costume::Costume(json data) : Asset(data) {
	...
	u8* textureData = 0;
	size_t size = 0;
	Executor::instance->zfile->getContentForFile(this->md5ext, &textureData, &size, true);
	SDL_RWops *rw = SDL_RWFromMem(textureData, size);

	if (strcmp(dataFormat, "svg") == 0) {
		SDL_Surface* srf = IMG_LoadSVG_RW(rw);
		this->costumeTexture = SDL_CreateTextureFromSurface(Game::instance->renderer, srf);
		SDL_FreeSurface(srf);
	}
	else {
